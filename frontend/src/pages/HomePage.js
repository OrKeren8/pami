...
const HomePage = () => {
  ...
  const handleCreateSlackChannel = async (e) => {
    ...
    try {
      const response = await slackApi.post("/slack/channels", {
      ...
      <>
        ...
        >
          ...
          <p style={{ color: "#666", marginTop: "10px" }}>
            Use the server-side Slack bot credentials to connect once, then
            continue to Slack actions.
          </p>
        </div>
  ...
  return (
    <div
      className={`dashboard-container ${isSidebarOpen ? "sidebar-open" : "sidebar-closed"}`}
    >
