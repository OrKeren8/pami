...
from projects_service.services.ai_conversation_service.service import (
...
class TestAIConversationService:
    ...
    async def test_create_conversation(self, service, mock_s3_client):
        ...
        # Act
        conversation = await service.create_conversation(context_node_id, project_id, title)
    ...
    @pytest.mark.asyncio
    async def test_send_message_success(self, service, mock_s3_client, mock_bedrock_client):
        # Arrange
        ...
        mock_s3_client.get_object.return_value = {
            'Body': MagicMock(read=MagicMock(return_value=str(conversation_data).encode()))
        }

        # Mock Bedrock response
        mock_bedrock_client.invoke_model.return_value = {
            'body': MagicMock(read=MagicMock(return_value=b'{"outputs": [{"text": "Hello human!"}]}'))
        }

        # Act
        response = await service.send_message(conversation_id, user_message, context_snapshot)
    ...
    async def test_list_conversations_for_node(self, service, mock_s3_client):
        ...
        mock_s3_client.list_objects_v2.return_value = {
            'Contents': [
                ...
                {'Key': f'conversations/other_node/conv3.json'}  # Should be filtered out
            ]
        }
    ...
    async def test_call_bedrock_ai(self, service, mock_bedrock_client):
        # Arrange
        messages = [
            Message(role="user", content="Hello AI", timestamp=datetime.utcnow())
        ]
    ...
    async def test_call_bedrock_ai_error_handling(self, service, mock_bedrock_client):
        # Arrange
        messages = [Message(role="user", content="Hello", timestamp=datetime.utcnow())]
